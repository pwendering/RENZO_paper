...
data_file_base_name = 'Clb2_global_v2';

%{
excel_file = ['../Data/' data_file_base_name '.ods'];
prot_abundance_sheet = 'Clb2_global';
diff_genes_sheet = 'D';
...
biomass_ratio_L = 4.1/4.7;
biomass_ratio_D = 4.1/4.7;

% protein content
protein_ratio_m_L = 20; % Light
protein_ratio_m_D = 21.3; % Dark

protein_ratio_w_L = 19.9; % Light
protein_ratio_w_D = 19.8; % Dark
...
% include beta-carotene in biomass reaction
result = RENZO(model_mut,model_wild,msrd_proteins_swissprot,...
...
% find overlap with genes found by Filipp
Genes_Filipp = importdata(['../Data/' data_file_base_name '_D.csv']);
Genes_Filipp = Genes_Filipp(2:end);
% light
found_in_Filipp_light = zeros(length(genes_in_distinc_rxns_light),1);
count = 0;
for i = 1:length(genes_in_distinc_rxns_light)
    for j = 1:length(Genes_Filipp)
        if strcmp(Genes_Filipp{j},model.genes{genes_in_distinc_rxns_light(i)})
            count = count + 1;
            found_in_Filipp_light(i) = 1;
            break
        end
    end
end

writetable(array2table(found_in_Filipp_light,...
    'VariableNames',{'found_bool'},...
    'RowNames',model.genes(genes_in_distinc_rxns_light)),...
    [out_dir filesep 'diff_genes_' strtok(data_file_base_name,'_') '_light.csv'],...
    'QuoteStrings',false,'Delimiter','\t','WriteRowNames',true)

% dark
found_in_Filipp_dark = zeros(length(genes_in_distinc_rxns_dark),1);
count = 0;
for i = 1:length(genes_in_distinc_rxns_dark)
    for j = 1:length(Genes_Filipp)
        if strcmp(Genes_Filipp{j},model.genes{genes_in_distinc_rxns_dark(i)})
            count = count + 1;
            found_in_Filipp_dark(i) = 1;
            break
        end
    end
end

writetable(array2table(found_in_Filipp_dark,...
    'VariableNames',{'found_bool'},...
    'RowNames',model.genes(genes_in_distinc_rxns_dark)),...
    [out_dir filesep 'diff_genes_' strtok(data_file_base_name,'_') '_dark.csv'],...
    'QuoteStrings',false,'Delimiter','\t','WriteRowNames',true)
